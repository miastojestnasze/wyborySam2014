...
urlpatterns = [
    ...
    url(r'^api/stats/(?P<election_type>.+)/district-(?P<district>[\w|\W]+)/circle-(?P<number_of_district>[1-9]+)/circuit-(?P<number_of_electoral_circuit>[1-9]+)/$',
        get_stats, name='electoral-circuit-stats'),
    url(r'^api/stats/(?P<election_type>.+)/district-(?P<district>[\w|\W]+)/circuit-(?P<number_of_electoral_circuit>[1-9]+)/$',
        get_stats, name='district-stats'),
    url(r'^api/stats/(?P<election_type>.+)/district-(?P<district>[\w|\W]+)/circle-(?P<number_of_district>[1-9]+)/$',
        get_stats, name='electoral-circle-stats'),
    url(r'^api/stats/(?P<election_type>.+)/circle-(?P<number_of_district>[1-9]+)/$',
        get_stats, name='district-stats'), 
    url(r'^api/stats/(?P<election_type>.+)/circle-(?P<number_of_district>[1-9]+)/circuit-(?P<number_of_electoral_circuit>[1-9]+)/$',
        get_stats, name='district-stats'),
